...
static func do_01(node: Node, duration: float, on_update: Callable) -> Tween:
	if node == null or duration < 0.0: return null
	var t := node.create_tween()
	t.tween_method(on_update, 0.0, 1.0, duration)
	return t

static func do(node: Node, start, end, duration: float, on_update: Callable) -> Tween:
	if node == null or duration < 0.0: return null
	var t := node.create_tween()
	t.tween_method(func(f: float) -> void: on_update.call(lerp(start, end, f)), 0.0, 1.0, duration)
	return t

static func do_01_curve(node: Node, duration: float, curve: Curve, on_update: Callable) -> Tween:
...
static func do_curve(node: Node, start, end, duration: float, curve: Curve, on_update: Callable) -> Tween:
	if node == null or duration < 0.0 or curve == null: return null
	var t := node.create_tween()
